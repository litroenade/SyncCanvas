...
from src.agent.lib.canvas.helpers import base_excalidraw_element, get_theme_colors, generate_element_id
...
def create_test_node(label: str, node_type: str, x: float, y: float, theme: str = "light"):
    """创建测试节点的辅助函数"""
...
def test_flowchart_creation():
    ...
    edges = [{"from_id": f"step_{i}", "to_id": f"step_{i+1}"} for i in range(len(steps)-1)]
...
def test_node_types():
    ...
    for node_type, expected_shape, label in node_types:
        ...
        assert actual_shape == expected_shape, f"节点类型{node_type}应该是{expected_shape}"
...
def test_element_creation():
    ...
    for elem_type, name in element_types:
        element = base_excalidraw_element(
            elem_type, 100, 100, 150, 80, "#000", "#fff"
        )
        ...
        assert element['isDeleted'] == False
...
def test_element_properties():
    ...
    required_props = [
        ...
        'angle', 'strokeColor', 'backgroundColor',
        'fillStyle', 'strokeWidth', 'strokeStyle',
        'roughness', 'opacity', 'isDeleted', 'groupIds',
...
def test_coordinate_system():
    ...
    # 创建一系列节点测试坐标
    nodes = [{"id": l, "label": l} for l in ["A", "B", "C"]]
